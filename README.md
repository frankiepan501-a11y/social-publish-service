...
- 2026-07-16 FB/IG image-task product-reference copy:
  ...
  - Follow-up fix: newly-created image-task records can briefly return Feishu `1254607 Data not ready`; GET reads now retry this response before surfacing an error. Feishu JSON responses are parsed as UTF-8 to avoid Chinese select values being mis-decoded when the response lacks an explicit charset.
  - Follow-up hardening: image-task status parsing now includes a narrow CP949/GBK mojibake repair path for worker status fields only, so pending worker states remain pending no-ops even if an upstream/runtime layer presents a mis-decoded status string.
  - Follow-up hardening: image-result ingest now avoids writing non-http worker locations into the URL-styled `AI生成图链接` field and retries content writeback without optional `运行/回放ID` when older production tables do not have that field.
  - Verification: targeted local tests passed: `tests.test_image_task`, `tests.test_feishu_client`, and the image-task generation tests for dry-run payloads, reference budget, missing-reference blocking, write-gate blocking, scan selection, pending ingest no-op, and successful inline worker-result ingest.
...
- 2026-07-16 Powkong Meta/IG production canary:
  ...
  - Fix: use a public publishable asset URL for the canary, configure the Powkong Page access token and verified Page/IG IDs, wait for Instagram containers before publish and retry transient `9007`, and stop reading Facebook `post_id` after `/photos` succeeds. Code commits: `163dcea` and `31b3821`.
  - Verification: local `unittest discover -s tests -v` passed 107 tests with service-token env cleared for local auth; Zeabur deployment `31b3821` reached `RUNNING`; `/health` returned `ok=true`, `meta_configured=true`, and both `commit_enabled=false` / `asset_prepare_enabled=false` after the canary window closed. Powkong IG published content-calendar record `recvpqW1TY4ofK` with media `18096903065179301` and permalink `https://www.instagram.com/p/Da2Kqi9oFVy/`. Powkong FB Page published record `recvpqW2wdnrxt` with photo `122203869050467548` and link `https://www.facebook.com/photo.php?fbid=122203869050467548&set=a.122097563354467548&type=3`; the Base row was manually corrected from the old `post_id` lookup failure to `已发布` after Graph verification.
  - Safety state: publish n8n remains inactive; account-table auto-publish remains disabled/dry-run by default; production canary used direct single-record `/publish/commit` with inline account config, and the commit/material-preparation gates were closed immediately afterward.
  - Follow-up on 2026-07-16: the FB test photo was deleted through Graph API and its Base row was marked `已驳回` with `CANARY_DELETED`; IG delete returned Graph code `10 Insufficient permissions` with both available tokens, so Zhang Jiaye was asked to delete the IG test post manually. The four account rows were moved into P2 gray state by setting `自动发布启用=true` and `默认发布模式=auto`; per-account caps remain conservative (`每日上限=1`, `每周上限=3`, min interval 24h for FB and 36h for IG). `/health` still showed `commit_enabled=false` and `asset_prepare_enabled=false`, `/publish/scan {"commit":false}` selected zero candidates, and publish workflow `dLwVZJiP580NGGDL` remained inactive.
  - Follow-up on 2026-07-16 P2 four-account live run: real publishing now uses brand-specific Page tokens from `META_ACCESS_TOKEN_POWKONG` and `META_ACCESS_TOKEN_FUNLAB`; the global `META_ACCESS_TOKEN` remains only a fallback. Code commit `3082e8f` also changed Facebook photo lookup to read only `id,link`, because `permalink_url` is not available on the photo object in this Graph API path. Local targeted tests (`tests.test_config`, `tests.test_meta_client`) passed, and all remaining groups (`test_rules`, `test_feishu_client`, `test_remote_readiness`, `test_approval`, `test_planning`, `test_generation`) passed. Production manual `/publish/scan {"commit":true,"limit":20}` committed two restored FB rows with `failed=0`; together with the earlier IG rows, the four P2 records are now published: Powkong IG `recvpwugmPPQoK` -> `https://www.instagram.com/p/Da2OId8IE10/`, Powkong FB `recvpwugPD7xQO` -> `https://www.facebook.com/photo.php?fbid=122203872062467548&set=a.122097563354467548&type=3`, FUNLAB IG `recvpwuhgKO0XL` -> `https://www.instagram.com/p/Da2ONBxIMjN/`, FUNLAB FB `recvpwuhI8CpWL` -> `https://www.facebook.com/photo.php?fbid=887260437775419&set=a.116671061501031&type=3`. Graph API readback confirmed all four media/photo IDs. After the run, `/health` returned `commit_enabled=false`, `asset_prepare_enabled=false`, `meta_configured=true`, and a final dry-run scan selected zero records.
- 2026-07-20 Meta P0 permissions follow-up:
  - Result: Facebook/Instagram organic publish remains verified for both POWKONG and FUNLAB from the four-account live run, and `/health` currently returns `meta_configured=true` with `commit_enabled=false` / `asset_prepare_enabled=false` as the safe default.
  - Problem: four-account read-only `/insights/poll` smoke returned opaque HTTP 500 before this fix. After deploy, the same smoke now returns structured Meta errors instead of black-box 500: IG records return Graph code `10 Application does not have permission for this action`; FB photo records return Graph code `100 Unsupported get request` when only a photo ID is available.
  - Fix: IG media insights metric now uses Meta official `saved` metric name instead of `saves`; `/insights/poll` falls back from `Meta Page Post ID` to `FB Photo ID` / `平台返回ID` and catches `MetaApiError` as a structured blocked result.
  - Verification: `tests/test_meta_client.py` passed 5 tests, `py_compile` passed for `app/main.py` and `app/meta_client.py`, commit `cd9f68c` was deployed, and post-deploy four-account evidence was written to `D:\Documents\社交媒体\fbig_evidence\meta_p0_permissions_20260720\insights_poll_four_accounts_after_patch_20260720.json`.
  - Remaining permission gap: to mark Meta insights P0 complete, request/confirm `instagram_manage_insights` for IG and add a reliable Facebook Page post-ID or Page/post insights path. Do not mix this with Meta Ads permissions.
...
- 2026-07-15 FB/IG card closure hardening:
  - Problem: weekly strategy and approval-card callbacks could write Base but still leave the original Feishu card unchanged; the approval review card also exposed only regenerate actions even though the service supports `approve_schedule` and `reject`.
  - Fix: `scripts/send_approval_card_v3.py` now renders all five service actions (`approve_schedule`, `regenerate_image`, `regenerate_copy`, `regenerate_both`, `reject`) with stable `value.action=fbig_image_feedback` payload fields. n8n Event Hub workflow `YjTXaoWAcy89xZpT` now passes weekly-card `message_id/open_message_id/open_chat_id/operator_open_id` through the payload and both `FBIG Weekly Input Reply` and `FBIG Approval Reply` PATCH the original card first, falling back to an interactive card only if PATCH fails.
  - Verification: n8n readback confirmed weekly/approval reply nodes contain PATCH-original-card logic and no longer send plain-text receipts, while daily-confirm and KOL reply nodes still retain PATCH logic. Local `unittest discover -s tests` passed 98 tests with service-token env cleared for local auth. Production canary locked four weekly strategies, created 8 weekly candidates, pushed 1 daily-confirm candidate into content calendar, then used an existing asset record for `approve_schedule -> /publish/scan {"commit":false}`; dry-run returned `selected=1 / dry_run_passed=1 / committed=0 / failed=0`. The canary record was restored to `状态=待审核`, approval flags false, no schedule, no publish account.
- 2026-07-15 FB/IG image-worker queue terminal-state hardening:
  - Problem: sample/test image tasks could remain in `待处理` and keep `/image-task/ingest-scan` reporting a pending queue even after operators no longer intended to process them.
...
- `META_ACCESS_TOKEN`
- `META_ACCESS_TOKEN_POWKONG` (optional brand-specific Page token; preferred for Powkong records)
- `META_ACCESS_TOKEN_FUNLAB` (optional brand-specific Page token; preferred for FUNLAB records)
- `FEISHU_APP_ID`
- `FEISHU_APP_SECRET`
- `FEISHU_BASE_TOKEN`
