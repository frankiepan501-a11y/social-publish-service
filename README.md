...
- 2026-07-20 Meta P0 permissions follow-up:
  ...
  - Problem: four-account read-only `/insights/poll` smoke returned opaque HTTP 500 before this fix. After deploy, the same smoke now returns structured Meta errors instead of black-box 500. After commit `f871204`, IG records return Graph code `10 Application does not have permission for this action`; FB records return Graph code `190 Invalid OAuth 2.0 Access Token`, so the remaining Facebook data issue is token/object access plus a reliable Page post/photo insights path.
